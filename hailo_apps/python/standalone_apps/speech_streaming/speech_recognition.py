...
Simple CLI app: read from stdin or load an audio file, transcribe with Whisper
...
Usage:
    # Read from stdin (Ctrl+C to stop):
...
def _transcribe_file(audio_path: str, pipeline, chunk_length: int):
    ...
    print(f"✓ Loaded ({duration:.1f}s, {len(audio)} bytes)")
